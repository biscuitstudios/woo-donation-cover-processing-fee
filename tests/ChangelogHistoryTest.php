 ...
 * newest release stops every site being offered anything, silently, which is the
...
final class ChangelogHistoryTest extends TestCase {
	...
	private const MAIN = __DIR__ . '/../woo-cover-processing-fee.php';

	protected function tearDown(): void {
		unset( $GLOBALS['_test_http'], $GLOBALS['_test_http_code'] );
	}

	/** Calls a private method on a real instance. */
	private static function call( WOO_Donation_Cover_Fee_Updater $updater, string $method, array $args = [] ) {
		$ref = new ReflectionMethod( WOO_Donation_Cover_Fee_Updater::class, $method );
	...
	private static function apiRelease( string $tag, string $published, string $body = '* A thing', array $overrides = [] ): array {
		return array_merge(
			[
				...
				'body'         => $body,
				'html_url'     => 'https://github.com/biscuitstudios/woo-donation-cover-processing-fee/releases/tag/' . $tag,
				'draft'        => false,
				'prerelease'   => false,
				'assets'       => [
					[
	...
	private static function fetch( array $releases ) {
		$GLOBALS['_test_http'] = (string) json_encode( $releases );

		return self::call( new WOO_Donation_Cover_Fee_Updater( self::MAIN ), 'fetch_release' );
	}
	...
	public function test_it_offers_the_newest_release(): void {
		$release = self::fetch( [
			self::apiRelease( 'v2.7.0', '2026-09-14T10:00:00Z' ),
			self::apiRelease( 'v2.6.0', '2026-09-11T10:00:00Z' ),
		] );
	...
	public function test_a_newest_release_with_no_zip_offers_nothing(): void {
		...
		$release = self::fetch( [
			self::apiRelease( 'v2.7.0', '2026-09-14T10:00:00Z', '* A thing', [ 'assets' => [] ] ),
		] );
	...
	public function test_the_log_is_newest_first_and_carries_dates(): void {
		$release = self::fetch( [
			self::apiRelease( 'v2.7.0', '2026-09-14T10:00:00Z' ),
			self::apiRelease( 'v2.6.0', '2026-09-11T10:00:00Z' ),
			self::apiRelease( 'v2.5.0', '2026-09-08T10:00:00Z' ),
		] );

		$this->assertSame( [ '2.7.0', '2.6.0', '2.5.0' ], array_column( $release['log'], 'version' ) );
	...
	public function test_a_short_history_is_not_truncated(): void {
		$release = self::fetch( [ self::apiRelease( 'v2.7.0', '2026-09-14T10:00:00Z' ) ] );
	...
	public function test_one_request_serves_both_jobs(): void {
		self::fetch( [ self::apiRelease( 'v2.7.0', '2026-09-14T10:00:00Z' ) ] );
		...
		$this->assertStringContainsString( '/releases?per_page=11', $GLOBALS['_test_http_url'] );
	}
	...
	public function test_the_trailing_compare_link_is_stripped(): void {
		$release = self::fetch( [
			self::apiRelease(
				...
				"* A thing\n\n**Full Changelog**: https://github.com/biscuitstudios/woo-donation-cover-processing-fee/compare/v2.6.0...v2.7.0"
			),
		] );
	...
	public function test_a_compare_link_written_into_the_notes_survives(): void {
		...
		$release = self::fetch( [ self::apiRelease( 'v2.7.0', '2026-09-14T10:00:00Z', $body ) ] );
	...
	private static function render( array $release ): string {
		return self::call( new WOO_Donation_Cover_Fee_Updater( self::MAIN ), 'render_changelog', [ $release ] );
	}
	...
	private static function renderable( array $log, bool $truncated = false ): array {
		return [
			'log'          => $log,
			'truncated'    => $truncated,
			'releases_url' => 'https://github.com/biscuitstudios/woo-donation-cover-processing-fee/releases',
		];
	}

	public function test_every_version_gets_a_heading_with_its_date(): void {
		$html = self::render( self::renderable( [
			[ 'version' => '2.7.0', 'published' => '2026-09-14T10:00:00Z', 'notes' => '* A thing' ],
			[ 'version' => '2.6.0', 'published' => '2026-09-11T10:00:00Z', 'notes' => '* Another thing' ],
		] ) );

		$this->assertStringContainsString( '<h3>2.7.0 | September 14, 2026</h3>', $html );
		$this->assertStringContainsString( '<h3>2.6.0 | September 11, 2026</h3>', $html );
	}
	...
	public function test_a_version_heading_outranks_a_heading_inside_release_notes(): void {
		...
		$html = self::render( self::renderable( [
			[ 'version' => '2.3.0', 'published' => '2026-08-22T10:00:00Z', 'notes' => "## What's Changed\n* A thing" ],
		] ) );

		$this->assertStringContainsString( '<h3>2.3.0 | August 22, 2026</h3>', $html );
	...
	public function test_a_version_with_no_date_still_gets_a_heading(): void {
		$html = self::render( self::renderable( [
			[ 'version' => '2.7.0', 'published' => '', 'notes' => '* A thing' ],
		] ) );

		$this->assertStringContainsString( '<h3>2.7.0</h3>', $html );
	}
	...
	public function test_notes_fall_back_to_the_shipped_readme(): void {
		...
		$html = self::render( self::renderable( [
			[ 'version' => '2.6.0', 'published' => '2026-09-11T10:00:00Z', 'notes' => '' ],
		] ) );
	...
	public function test_the_footer_link_names_what_was_left_out(): void {
		$full = self::render( self::renderable( [ [ 'version' => '2.7.0', 'published' => '', 'notes' => '* A thing' ] ] ) );
		$cut  = self::render( self::renderable( [ [ 'version' => '2.7.0', 'published' => '', 'notes' => '* A thing' ] ], true ) );
	...
	public function test_nothing_renders_a_pre_tag(): void {
		...
		$html = self::render( self::renderable( [
			[ 'version' => '2.7.0', 'published' => '2026-09-14T10:00:00Z', 'notes' => "* A very long line that would otherwise run off the side of the modal and force the reader to scroll" ],
		] ) );
	...
	public function test_the_shipped_readme_parses(): void {
		...
		$sections = self::call( new WOO_Donation_Cover_Fee_Updater( self::MAIN ), 'readme_changelog' );

		$this->assertNotEmpty( $sections );
		$this->assertArrayHasKey( '2.6.0', $sections );
		$this->assertStringStartsWith( '*', $sections['2.6.0'] );
	}
	...
	public function test_the_changelog_read_stops_at_the_next_readme_section(): void {
		...
		$dir = sys_get_temp_dir() . '/woodon-readme-test-' . uniqid();
		mkdir( $dir );
		file_put_contents(
			$dir . '/readme.txt',
			"== Changelog ==\n\n= 1.0.0 =\n* A thing\n\n== Upgrade Notice ==\n\n= 1.0.0 =\nDo not swallow this.\n"
		...
		$sections = self::call( new WOO_Donation_Cover_Fee_Updater( $dir . '/plugin.php' ), 'readme_changelog' );
	...
	public function test_a_missing_readme_is_not_fatal(): void {
		$sections = self::call( new WOO_Donation_Cover_Fee_Updater( sys_get_temp_dir() . '/definitely-not-here/plugin.php' ), 'readme_changelog' );
