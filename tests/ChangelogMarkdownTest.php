...
final class ChangelogMarkdownTest extends TestCase {
	...
	private static function render( string $markdown ): string {
		return WOO_Donation_Cover_Fee_Updater::render_markdown( $markdown );
	}
	...
	public function test_code_spans_and_bold(): void {
		$html = self::render( '* The `woo_cover_fee_thing` filter is **new**' );

		$this->assertSame( '<ul><li>The <code>woo_cover_fee_thing</code> filter is <strong>new</strong></li></ul>', $html );
	}
