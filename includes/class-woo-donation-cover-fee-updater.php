...
class WOO_Donation_Cover_Fee_Updater {
	...
	const CACHE_KEY = 'woo_donation_cover_fee_updater_release_v2';
	...
	private function fetch_release() {
		...
		if ( ! is_array( $body ) || [] === $body ) {
			error_log( '[Woo Donation Cover Fee] Update check failed: no releases listed for ' . self::REPO );
		...
		if ( [] === $log ) {
			error_log( '[Woo Donation Cover Fee] Update check failed: no usable release tags for ' . self::REPO );
		...
		if ( '' === $package ) {
			error_log( '[Woo Donation Cover Fee] Update check failed: release v' . $latest['version'] . ' has no zip asset.' );
	...
	private function render_changelog( $release ) {
		...
		foreach ( $release['log'] as $entry ) {
			...
			$out .= '' === $notes
				? '<p>' . esc_html__( 'No release notes were published for this version.', 'woo-donation-cover-processing-fee' ) . '</p>'
		...
		$out .= sprintf(
			...
			$release['truncated']
				? esc_html__( 'Earlier releases are on GitHub', 'woo-donation-cover-processing-fee' )
				: esc_html__( 'View all releases on GitHub', 'woo-donation-cover-processing-fee' )
		);
